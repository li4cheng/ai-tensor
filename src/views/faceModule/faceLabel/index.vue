<template>
  <div class="app-container">
    <el-row class="main">
      ...
      <el-col :span="19" class="right-box">
        ...
        <el-form class="form" ref="listForm" :model="listQuery" :inline="true" @keyup.enter.native="getTableList">
          ...
          <el-form-item v-permission="'faceLabelMan:btn_delete_cluster'">
            <el-button size="mini" v-if="listQuery.num == 4" type="danger" @click="onEmpty" v-permission="'faceLabelMan:btn_delete_cluster'">一键删除陌生人</el-button>
          </el-form-item>
        </el-form>
        ...
        <div class="common-tool">
          ...
          <div class="right">
            <el-button class="txt-btn" size="mini" type="text" icon="el-icon-plus" @click="onItemEdit()" v-permission="'faceLabelMan:btn_add'">添加</el-button>
            <el-button class="txt-btn" size="mini" type="text" icon="el-icon-delete" @click="onItemDelete(listQuery.num)" v-if="listQuery.num != 2" v-permission="'faceLabelMan:btn_batchDelete'">{{listQuery.num == 4?'删除':'注销'}}</el-button>
            <div class="separator"></div>
            <el-button class="txt-btn" size="mini" type="text" icon="el-icon-upload" @click="onExportLabelZip" v-permission="'faceLabelMan:btn_download'">导出人脸包</el-button>
            <div class="separator"></div>
        ...
        <el-table v-show="mode == 0" ref="listTabel" :data="listData.rows" height="calc(100vh - 255px)" v-loading.body="listLoading" highlight-current-row @selection-change="onListChange">
          ...
          <el-table-column fixed="right" width="130" align="center" label="操作">
            <template slot-scope="scope">
              <el-button size="mini" type="success" @click="onItemEdit(scope.row.id)" v-permission="'faceLabelMan:btn_edit'">编辑</el-button>
              <el-button size="mini" type="primary" @click="onOpenLayer(scope.row)" v-permission="'faceLabelMan:btn_track'">轨迹</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!-- 表格模式 E -->
        ...
        <div class="card-list" v-show="mode == 1">
          <div class="item" :class="{sel: listIds.includes(item.id)}" v-for="item in listData.rows" :key="item.id" @click="$refs.listTabel.toggleRowSelection(item)">
            ...
            <div class="box">
              ...
              <div class="btns">
                <el-button size="mini" @click.stop="onItemEdit(item.id)" v-permission="'faceLabelMan:btn_edit'">编辑</el-button>
                <el-button size="mini" @click.stop="onOpenLayer(item)" v-permission="'faceLabelMan:btn_track'">轨迹</el-button>
                <el-button size="mini" @click.stop="onItemDetails(item)" v-permission="'faceLabelMan:btn_get_label'">详情</el-button>
              </div>
            </div>
          </div>
        </div>
    ...
    <label-form ref="labelForm" :is-open.sync="isFormDialog" :id="labelFormId" :labelLibrary="listQuery.num" @complete="updateView" :readonly="formReadonly" />
...
export default {
  ...
  methods: {
    ...
    onItemDelete(type) {
      ...
      let tip = type == 4 ? "删除" : "注销";
      this.$confirm(`此操作将永久${tip}, 是否继续?`, "提示", {
        cancelButtonText: "取消",
        confirmButtonText: "确定",
        ...
        deleteLabel({
          ...
          this.$notify({
            title: isSuccess ? "成功" : "失败",
            message: isSuccess ? `${tip}成功` : res.message,
            type: isSuccess ? "success" : "error",
            duration: 2000
          });
