    ...
    <el-row class="main">
      ...
      <el-col :span="19" class="right-box">
        ...
        <div class="common-tool">
          ...
          <div class="right">
            <el-button class="txt-btn" size="mini" type="text" icon="el-icon-plus" @click="onItemEdit()" v-permission="'cameraMan:btn_add'">添加</el-button>
            <el-button class="txt-btn" size="mini" type="text" icon="el-icon-delete" @click="onItemDelete" v-permission="'cameraMan:btn_batchDelete'">删除</el-button>
          </div>
        </div>
        ...
        <el-table ref="listTabel" :data="listData.rows" height="calc(100vh - 255px)" v-loading.body="listLoading" highlight-current-row @selection-change="onListChange">
          ...
          <el-table-column width="120px" align="center" label="位置照片">
            <template slot-scope="scope">
              <div class="pic" @click="onLookPicture(scope.row.cameraImageUrl)">
          ...
          <el-table-column width="120px" align="center" label="封面">
            <template slot-scope="scope">
              <div class="pic" @click="onLookPicture(scope.row.cameraBgImageUrl)">
                <img-show height="50px" txt="暂无图片" :img-src="scope.row.cameraBgImageUrl" />
              </div>
            </template>
          </el-table-column>
          <el-table-column label="名称/品牌" width="160">
            <template slot-scope="scope">
              <div class="info txt">
          ...
          <el-table-column label="WebServer" width="90">
            <template slot-scope="scope">
              <a :href="scope.row.cameraUrl" target="_blank" class="txt">点击跳转</a>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" width="150" align="center" label="操作">
            <template slot-scope="scope">
              <el-button size="mini" type="success" @click="onItemEdit(scope.row.id)" v-permission="'cameraMan:btn_edit'">编辑</el-button>
              <el-button size="mini" type="primary" @click="onTest(scope.row)" v-permission="'cameraMan:btn_close_test'">闭合测试</el-button>
              <el-button size="mini" type="danger" @click="onClearCache(scope.row.id)">清空缓存</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!-- 表格模式 E -->
...
<script>
import { getTest, deleteClearCache } from "@/api/camera/index";
...
export default {
  ...
  methods: {
    ...
    // 清空缓存
    onClearCache(id) {
      deleteClearCache(id).then(res => {
        let isSuccess = res.status === 200;
        this.$notify({
          title: isSuccess ? "成功" : "失败",
          message: isSuccess ? "清空成功" : res.message,
          type: isSuccess ? "success" : "error",
          duration: 2000
        });
      });
