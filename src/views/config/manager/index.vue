<template>
  <div>
    ...
    <div class="common-tool">
      ...
      <div class="right">
        <el-button class="txt-btn" v-permission="'configMan:btn_add'" size="mini" @click="onItemAdd" type="text" icon="el-icon-plus">添加</el-button>
        <!--（后续）
    ...
    <el-table :data="listData.rows" height="calc(100vh - 302px)" v-loading.body="listLoading" @selection-change="onListChange">
      ...
      <el-table-column width="250px" align="center" label="操作">
        <template slot-scope="scope">
          <el-button type="success" size="mini" v-permission="'configMan:btn_edit'" @click="onItemEdit(scope.row)">编辑</el-button>
          <el-button type="primary" size="mini" v-permission="'configMan:btn_get_cache'" @click="onCheckCache(scope.row.configName)">查看缓存</el-button>
          <el-button type="warning" size="mini" v-permission="'configMan:btn_clear_cache'" @click="onClear(scope.row.configName)">清理缓存</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    //     this.$notify({
    //       title:"提示",
    //       message:"请选择需要清除缓存的数据",
    //       type:"warning",
    //       duration:2000
    //     });
    //     return;
    //   }
    //   this.$confirm("确定清除所选项缓存?","提示",{
    //     cancelButtonText: "取消",
